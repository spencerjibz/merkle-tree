...
mod tree_with_fjall {
    ...
    use merkle_tree::{
        ...
        stores::{temporary_fjall_db, FjallDbStore, NodeStore},
    ...
    fn test_constructions() {
        ...
        let store = FjallDbStore::new(LazyLock::force(&FJALL_DB), "constructions").unwrap();
    ...
    fn test_verifing_proof() {
        (1_usize..100).for_each(|index| {
            ...
            let store = FjallDbStore::new(
                LazyLock::force(&FJALL_DB),
            ...
            for (h1, proof) in data.iter().zip(tree.proof_multiple(&data)) {
                assert!(MerkleTree::<FjallDbStore>::verify_proof(
                    h1, &proof, root_hash
                ))
            }
        });
    ...
    fn update_works() {
        let data = example_data(4);
        let store = FjallDbStore::new(LazyLock::force(&FJALL_DB), "proof_verification").unwrap();
        let mut tree = MerkleTree::construct(&data, store);
