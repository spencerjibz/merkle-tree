...
mod tree_with_sled_store {
    ...
    fn test_verifing_proof() {
        (1_usize..100).for_each(|index| {
            ...
            for (h1, proof) in data.iter().zip(tree.proof_multiple(&data)) {
                assert!(MerkleTree::<SledStore>::verify_proof(h1, &proof, root_hash))
            }
        });
    }
    #[test]
        ...
        (1_usize..100)
            ...
            .for_each(|index| {
                ...
                if let Some(proof) = tree.prove(&extra) {
                    assert!(MerkleTree::<SledStore>::verify_proof(
    ...
    fn verifies_data_set_forms_root() {
        ...
        for (input, hash) in pairs {
            ...
            let store =
                SledStore::new(LazyLock::force(&SLED_DB), "verifies_data_set_forms_root").unwrap();
    ...
    fn update_works() {
        let data = example_data(4);
        let store = SledStore::new(LazyLock::force(&SLED_DB), "proof_verification").unwrap();
        let mut tree = MerkleTree::construct(&data, store);
